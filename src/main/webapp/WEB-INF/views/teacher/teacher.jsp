...
<div id="tb">
    ...
    <a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-edit" onclick="openWinWithData('list','editWin','editForm','teacher');">编辑</a>
    <a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-edit" onclick="openClazzWin();">分配班级</a>
    <a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-edit" onclick="lookClazzWin();">查看班级</a>
    <div>
        <form id="searchForm">
            <input class="easyui-textbox easyui-validatebox" data-options="prompt:'请输入教师姓名',required:true,validType:'length[0,20]',novalidate:true" name="teacher.name" />
            <input class="easyui-textbox easyui-validatebox" data-options="prompt:'请输入教师电话',required:true,validType:'length[11,11]',novalidate:true" name="teacher.phone" />
    ...
    <form id="addForm">
        <table>
            ...
            <tr>
                ...
                <td>
                    <input  name="teacher.phone" class="easyui-textbox easyui-validatebox" data-options="prompt:'请输入电话',required:true,validType:'length[2,20]',novalidate:true" />
                </td>
            </tr>
            <tr>
                <td></td>
...
<div id="editWin" class="easyui-window normal-win" title="修改教师" data-options="closed:true">
    <form id="editForm">
        ...
        <table>
            ...
            <tr>
                <td></td>
                <td><a class="easyui-linkbutton" href="javascript:void(0);" onclick="update('/teacher/update', 'editForm', 'list', 'editWin')">修改</a></td>
            </tr>
        </table>
    </form>
</div>

<div id= "clazzWin" class="easyui-window large-win" title="分配教师" data-options="closed:true">

    <div id="clazzTb">
        <a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-add" onclick="okClazz();">确认选择</a>
        <div>
...
<script>
    ...
    function okClazz() {
        ...
        if (rows !== null && rows.length > 0) {
            ...
            $.post(contextPath + "/teacher/save_clazz",
                {
                    'teacherId':row.id,
                    'clazzIds':classIds
                },
    ...
    function lookClazzWin() {
        var row = $("#list").datagrid("getSelected");
        if (row) {
            openWin("clazzInfoWin");
            $("#clazzInfoList").datagrid({
                url:contextPath + '/teacher/clazz',
                method:'post',
                queryParams:{
                    teacherId:row.id
                }
