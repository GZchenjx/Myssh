...
function openWinWithData(listId, winId, formId, prefix) {
    var row = $("#" + listId).datagrid("getSelected");
    if (row) {
        var data = datagirdRowHandler(row, prefix);
        $("#" + formId).form("load", data);
        openWin(winId);
    } else {
        showAlert("请选中要修改的那一行");
    }
}
function datagirdRowHandler(row, prefix) {
    var data = {};
    for (var key in row) {
...
function saveOrUpdate(url, formId, listId, winId) {
    if (formValidate(formId)) {
        $.post(contextPath + url,
            $("#" + formId).serialize(),
            function (data) {
...
function save(url, formId, listId, winId) {
    saveOrUpdate(url,formId,listId,winId);
    $("#"+formId).form("clear");
}

function update(url, formId, listId, winId) {
    saveOrUpdate(url,formId,listId,winId);
    $("#"+formId).form("clear");
}
